...
//Print calculated local adresses to the lcd screen
void printAdressesLCD()
{
	LCDclear();
	std::string output = "";
	...
	LCDdrawstring_P(0, 0, output.c_str());
	LCDdisplay();
}

//Prints a string to the lcd screen
void printStringLCD(std::string output)
{
	LCDclear();
	LCDdrawstring_P(0,0, output.c_str());
	LCDdisplay();
...
//Prints the TritechLogo
void printTritechLCD()
{
	LCDclear();
	LCDdrawbitmap(0, 0, image_data_tritech, 84, 48, 255);
	LCDdisplay();
}


//Prints a car image to the lcd screen
void printCarLCD(int x, int y)
{
	LCDclear();
	if( x < 0 || y < 0)
		LCDdrawbitmapSpecial(x, y, image_data_car, 84, 48, 255);
	else
		LCDdrawbitmap(x,y, image_data_car, 84 , 48 , 255);
	LCDdisplay();
}

//Calculates local inet adresses
...
//Reads an ADC value from the ADC module
...
void *adcReadThread(void *ptr)
{
	...
	while(true)
	{
		...
		chann0String = getADCString(0);
		chann1String = getADCString(1);
		chann2String = getADCString(2);
		chann3String = getADCString(3);
...
//Handles all lcd drawing
void *lcdThread(void *ptr)
{
	long int start_time;
	struct timespec now;
	clock_gettime(CLOCK_REALTIME, &now);
	...
	while(lcdOn)
	{
		...
		if(splash)
		{
			...
			if(now.tv_sec - start_time > 10)
				splash = false;
			else if(now.tv_sec - start_time > 8)
				go = true;
			delay(23);
		...
		else
		{
			...
			if(!globalConnected)
			{
				...
				else
				{
					...
					lastCarbat = floor((chann0Double * 14.19 / 3.256) * 100) / 100;
					lastPibat = floor((chann2Double * 14.19 / 3.256) * 100) / 100;
					
					std::ostringstream strs;
					strs << "Car: " << lastCarbat << "V\nPi: " << lastPibat  << "V\n";
					output += strs.str();
			...
			else if(!lcdConfig) //No config recieved at all
			{
				std::string output = "Car server\nConnected\nNeed config";
				printStringLCD(output);
			}
...
int main()
{
	...
	pthread_create(&adc_thread, NULL, adcReadThread, NULL);
	pthread_create(&lcd_thread, NULL, lcdThread, NULL);
